...
export async function POST(request: Request) {
  ...
  Each event's headline should be a concise summary of the event in less than 10 words. The text should provide detailed information. Both the headline and text fields can include HTML markup for improved display. The group field should always be "${pageName}". Optionally, include a media field with a URL to the relevant section of the Wikipedia page and, if available, use the thumbnail image URL from the wiki summary as the "thumbnail".
  
  For example, use the URL format "https://en.wikipedia.org/wiki/${pageName}" for the media URL, and include the "thumbnail" if available from the wiki summary.
