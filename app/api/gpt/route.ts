...
export async function POST(request: Request) {
  ...
  Based on this content, identify and list the most important events related to "${pageName}", ranked by significance, not exceeding 30 entries, but also please NOT oversimplified
  ...
  2. Always include the "born" and "education" event if found.
  ...
  try {
    ...
    const events = (parsedContent.events || parsedContent).map((event: any) => ({
      ...event,
      group: polishedPageName,
      media: {
