...
async function generateFavicon() {
  try {
    ...
    await sharp(pngBuffer).resize(48, 48).toFile("public/favicon.ico");

    // Also save to app directory for Next.js - increased to 48x48 for better visibility
    await sharp(pngBuffer).resize(48, 48).toFile("app/favicon.ico");

    // Save larger versions for different devices
    await sharp(pngBuffer).resize(192, 192).toFile("public/icon-192.png");

    await sharp(pngBuffer).resize(512, 512).toFile("public/icon-512.png");
