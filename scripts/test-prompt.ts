...
async function getSystemPrompt(): Promise<string> {
  ...
  const promptPath = path.join(
    process.cwd(),
    "scripts/prompts/timeline-generator.txt",
  );
  try {
      ...
      timeline: {
        ...
        properties: {
          title: {
            ...
            description:
              "Concise description that states the subject's name, years (if known), nationality/background, and primary significance. For events/periods, state what it is and its historical importance.",
          },
          birthDate: {
            type: SchemaType.STRING,
            description:
              "Birth date in YYYY-MM-DD, YYYY-MM, or YYYY format. Only include for biographical timelines.",
          },
          deathDate: {
            type: SchemaType.STRING,
            description:
              "Death date in YYYY-MM-DD, YYYY-MM, or YYYY format. Only include for biographical timelines if the person is deceased.",
          },
          events: {
            type: SchemaType.ARRAY,
            description:
              "Array of chronologically ordered events, typically 15-30 events (up to 50 for broad historical topics)",
            items: {
              type: SchemaType.OBJECT,
              properties: {
                headline: {
                  type: SchemaType.STRING,
                  description:
                    "Concise, self-contained title describing the event",
                },
                description: {
                  type: SchemaType.STRING,
                  description:
                    "2-3 focused sentences that synthesize and paraphrase key information. Do NOT directly quote Wikipedia. Explain significance and impact, include essential context, and use information-dense language. Focus on what matters to the overall narrative.",
                },
                startDate: {
                  type: SchemaType.STRING,
                  description:
                    "Event date in YYYY-MM-DD, YYYY-MM, or YYYY format. For BCE dates, use negative years with leading zeros",
                },
                endDate: {
                  type: SchemaType.STRING,
                  description:
                    "Optional end date for events spanning a period, using same format as startDate",
                },
              },
...
Main content to extract events from: ${JSON.stringify(contentChunk)}`;
  ...
  try {
    ...
    if (error instanceof Error && error.message === "MAX_TOKENS_REACHED") {
      ...
      console.log(
        "total events",
        firstTimeline.timeline.events.length +
          secondTimeline.timeline.events.length,
      );
      // Merge the timelines
      return {
        timeline: {
          ...
          birthDate:
            firstTimeline.timeline.birthDate ||
            secondTimeline.timeline.birthDate,
          deathDate:
            firstTimeline.timeline.deathDate ||
            secondTimeline.timeline.deathDate,
          events: [
...
async function main() {
  ...
  try {
    ...
    await fs.writeFile(outputPath, JSON.stringify(timeline, null, 2), "utf-8");
