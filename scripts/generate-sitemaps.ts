...
async function generateSitemaps(inputFiles: string[]) {
  try {
    ...
    try {
      const sitemapIndexContent = await fs.readFile(
        "public/sitemap.xml",
        "utf-8",
      );
      const matches = sitemapIndexContent.match(/timelines-(\d+)\.xml/g);
      if (matches) {
        currentSitemapCount = Math.max(
          ...matches.map((m: string) => parseInt(m.match(/\d+/)![0])),
        );
      }
    } catch (error) {
    ...
    if (currentSitemapCount > 0) {
      try {
        lastSitemapContent = await fs.readFile(
          `public/sitemaps/timelines-${currentSitemapCount}.xml`,
          "utf-8",
        );
        const urls =
          lastSitemapContent
    ...
    if (
      ...
      lastSitemapUrls.size >= SITE_CONFIG.URLS_PER_SITEMAP / 2
    ) {
      currentFileIndex++;
      await fs.writeFile(
      ...
      const newContent = lastSitemapContent.replace(
        "</urlset>",
        pageNames
          .map(
            (pageName) => `
